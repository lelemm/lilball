...
pub fn resolve_walls(ball: &mut Ball, bounds: &Bounds) -> Vec<Impact> {
    ...
    if ball.pos.x - r < bounds.left {
        ...
        impacts.push(Impact { point: Vec2::new(bounds.left, ball.pos.y), normal: n, speed });
    }
    // Right wall.
    if ball.pos.x + r > bounds.right {
        ...
        let n = Vec2::new(-1.0, 0.0);
        ball.apply_impact(n, impulse_for(speed));
        impacts.push(Impact { point: Vec2::new(bounds.right, ball.pos.y), normal: n, speed });
    }
    // Top wall.
    if ball.pos.y - r < bounds.top {
        ...
        ball.apply_impact(n, impulse_for(speed));
        impacts.push(Impact { point: Vec2::new(ball.pos.x, bounds.top), normal: n, speed });
    }
    // Bottom wall.
    if ball.pos.y + r > bounds.bottom {
        ...
        let n = Vec2::new(0.0, -1.0);
        ball.apply_impact(n, impulse_for(speed));
        impacts.push(Impact { point: Vec2::new(ball.pos.x, bounds.bottom), normal: n, speed });
    }
