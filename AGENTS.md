...
In-app controls: left-drag to grab/throw the ball, sweep the cursor quickly
across the spring to briefly entangle it, right-click or `C` to cut/recall the
spring, `N` to fling it in a random direction, `G` to toggle gravity,
`R`/`Space` to reset, `Esc` to quit.
